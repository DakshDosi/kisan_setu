(() => {
  ...
  async function askBackend(question, locale) {
   ...
   function playAudio(audioBlob) {
    // Create a new URL from the audio blob
  ...
  elements.ask?.addEventListener('click', async () => {
    ...
    try {
      const audioBlob = await askBackend(question, locale);
      
      // We no longer get text, so we can't display it in the response box
      elements.responseBox.textContent = 'Playing audio response...';
      ...
      // const answer = data.final_answer || data.answer || data.gemini_answer_en || 'No response from backend';
      // elements.responseBox.textContent = answer;

      // // Animate + update status
